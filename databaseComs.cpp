...
void DatabaseComs::addSave(Character& karakter){
    ...
    command = "INSERT INTO Character "
              "(characterName, totalKillCount, itemMostUsed, entityMostUsed) ";
              "VALUES ('";
    //int charIdx = 0;
    ...
    //std::cout << command << "\n";
    execute(command);
    int charIdx = query.lastlastInsertId().toInt(); // get autogenerated Id
    ...
    for (int i = 0; i < karakter.collection.size(); ++i){
	    ...
	    //std::cout << command << "\n";
        execute(command);
        int entId = query.lastIsertId().toInt(); // get autogenerated Id
